    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool
    {
        if (!($sender instanceof Player) || !$sender->isOp()) {
            switch (strtolower(array_shift($args))):
                ...
                case 'join':
                    if (!(count($args) < 0b11)) {
                        $sender->sendMessage(TextFormat::RED . 'Usage: /sw ' . TextFormat::GREEN . 'join [SWname]' . TextFormat::GRAY . ' [PlayerName]');
                        break;
                    }
                    ...
                    if (isset($args[0])) {
                        ...
                        if (!array_key_exists($SWname, $this->pg->arenas)) {
                            $sender->sendMessage(TextFormat::RED . 'Arena with name: ' . TextFormat::WHITE . $SWname . TextFormat::RED . ' doesn\'t exist');
                            break;
                        }
                    } else {
                    ...
                    if (strlen($player) > 0 && $sender instanceof \pocketmine\command\ConsoleCommandSender) {
                        ...
                        if ($p instanceof Player) {
                            if ($this->pg->inArena($p->getName())) {
                                $p->sendMessage(TextFormat::RED . 'You are already inside an arena');
                                break;
                            }
                ...
                case 'quit':
                    if (!empty($args)) {
                        $sender->sendMessage(TextFormat::RED . 'Usage: /sw ' . TextFormat::GREEN . 'quit');
                        break;
                    }
                ...
                default:
                    ...
                    $sender->sendMessage(TextFormat::RED . 'Usage: /sw [join|quit]');
                    break;
            ...
            case 'create':
                ...
                */
                if (!(count($args) > 0b11 && count($args) < 0b101)) {
                    $sender->sendMessage(TextFormat::RED . "Usage: /sw " . TextFormat::GREEN . "create [SWname] [slots] [countdown] [maxGameTime]");
                    break;
                }
                ...
                foreach ($this->pg->arenas as $aname => $arena) {
                    if ($arena->getWorld() == $world) {
                        $sender->sendMessage(TextFormat::RED . 'You can\'t create 2 arenas in the same world try:');
                        $sender->sendMessage(TextFormat::WHITE . '/sw list' . TextFormat::RED . ' for a list of arenas');
                        $sender->sendMessage(TextFormat::WHITE . '/sw delete' . TextFormat::RED . ' to delete an arena');
                        unset($fworld, $world);
                        break 2;
                    }
                }
                ...
                foreach ($this->pg->signs as $loc => $name) {
                    if (explode(':', $loc)[3] == $world) {
                        $sender->sendMessage(TextFormat::RED . 'You can\'t create an arena in the same world of a join sign:');
                        $sender->sendMessage(TextFormat::WHITE . '/sw signdelete' . TextFormat::RED . ' to delete signs');
                        unset($fworld, $world);
                        break 2;
                    }
                }
                ...
                if (!($SWname && preg_match('/^[a-z0-9]+[a-z0-9]$/i', $SWname) && strlen($SWname) < 0x10 && strlen($SWname) > 0b10)) {
                    $sender->sendMessage(TextFormat::AQUA . '[SWname]' . TextFormat::RED . ' must consists of a-z 0-9 (min3-max15)');
                ...
                if (array_key_exists($SWname, $this->pg->arenas)) {
                    $sender->sendMessage(TextFormat::RED . 'Arena with name: ' . TextFormat::WHITE . $SWname . TextFormat::RED . ' already exist');
                    unset($fworld, $world, $SWname);
                    break;
                }
                ...
                if (!($slot && is_numeric($slot) && is_int(($slot + 0)) && $slot < 0x33 && $slot > 1)) {
                    $sender->sendMessage(TextFormat::AQUA . '[slots]' . TextFormat::RED . ' must be an integer >= 50 and >= 2');
                ...
                if (!($countdown && is_numeric($countdown) && is_int(($countdown + 0)) && $countdown > 0b1001 && $countdown < 0x12d)) {
                    $sender->sendMessage(TextFormat::AQUA . '[countdown]' . TextFormat::RED . ' must be an integer <= 300 seconds (5 minutes) and >= 10');
                ...
                if (!($maxtime && is_numeric($maxtime) && is_int(($maxtime + 0)) && $maxtime > 0x12b && $maxtime < 0x259)) {
                    $sender->sendMessage(TextFormat::AQUA . '[maxGameTime]' . TextFormat::RED . ' must be an integer <= 600 (10 minutes) and >= 300');
                ...
                if ($fworld == $world) {
                    ...
                    $sender->sendMessage(TextFormat::RED . 'There is a problem with the world name, try to restart your server');
                ...
                $pk->message = TextFormat::LIGHT_PURPLE . 'I\'m creating a backup of the world ' . TextFormat::AQUA . $world . TextFormat::LIGHT_PURPLE . ', do not move';
