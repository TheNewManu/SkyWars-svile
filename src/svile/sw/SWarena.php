...
final class SWarena
{
    ...
    {
        if ($slot > $this->slot) {
            $player->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'This arena have only got ' . TextFormat::WHITE . $this->slot . TextFormat::RED . ' slots');
            return false;
        }
        ...
        if (!$config->save() || count($this->spawns) != $this->slot) {
            $player->sendMessage(TextFormat::AQUA . 'â' . TextFormat::RED . 'An error occured setting the spawn, pls contact the developer');
            return false;
        } else
            return true;
    }
        ...
        foreach ($this->pg->getServer()->getLevelByName($this->world)->getTiles() as $tile) {
            if ($tile instanceof Chest) {
                ...
                foreach (array_shift($contents) as $key => $val) {
                    $tile->getInventory()->setItem($key-1, Item::get($val[0], 0, $val[1]));
                }
            }
        }
        unset($contents, $tile);
    ...
    public function stop($force = false)
    {
        ...
        foreach ($this->players as $name => $spawn) {
			...
			$this->pg->getServer()->broadcastMessage($p->getName() . " Âaha vinto le SkyWars");
